...
module.exports = function (app, connection, logger, bodyParser, https, parser) {
	...
	app.get('/joggl', function(request, response) {
        ...
        https.get(toggl_options, function(res) {
            ...
            res.on('end', function() {
                ...
                logger.info('### ### ### ### ### Querying JIRA on - ' + jira_url);
                https.get(jira_options, function(res2) {
                    res2.on('data', function(data) {
                        jira_body += data.toString();
                    });
                    res2.on('end', function(res2) {
                        logger.info('### ### ### ### ### ### JIRA data fetch complete.');
                        if (!jira_body) {
                            logger.error('### ### ### ### ### ### Ill framed JQL query, exiting!');
                            result.status = 'failure';
                            result.message = 'problem framing JIRA JQL query -> ' + jira_url;
                            response.json(result);
                        }
                        ...
                        var toggl_data = JSON.parse(toggl_body).data;
